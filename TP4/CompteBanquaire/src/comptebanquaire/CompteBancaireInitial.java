...
public class CompteBancaireInitial implements Comparable<CompteBancaireInitial> {
    ...
    @Override
    public int compareTo(CompteBancaireInitial CBI) {
        int diff = this.numero - CBI.numero;
        if (diff==0) {
            return diff ;
        } else if (diff<0){
            return -1 ;
